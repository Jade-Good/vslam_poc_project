  ...
  vector<bool> &vbTriangulated)
{
  ...
  if (RH > 0.40)
    return ReconstructH(
      vbMatchesInliersH, H, mK, R21, t21, vP3D, vbTriangulated, 1.0, 50);
  else  // if(pF_HF>0.6)
    return ReconstructF(
      vbMatchesInliersF, F, mK, R21, t21, vP3D, vbTriangulated, 1.0, 50);
  ...
  float &parallax)
{
  ...
  for (size_t i = 0, iend = vMatches12.size(); i < iend; i++)
  {
    ...
    if (
      !isfinite(p3dC1.at<float>(0)) || !isfinite(p3dC1.at<float>(1)) ||
